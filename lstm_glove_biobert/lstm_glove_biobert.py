...
def load_embeddings(word_to_idx, embed_size=300, use_biobert=False):
    ...
    # Track which words we found in each source
    glove_words = set()
    biobert_words = set()
    ...
    if glove_file:
        try:
            ...
            print(f"Found {glove_found_words}/{len(word_to_idx) - 2} words in GloVe embeddings")
        except Exception as e:
    ...
    if use_biobert and biobert_tokenizer is not None:
        ...
        # Process words that weren't found in GloVe
        for word, idx in tqdm(word_to_idx.items(), desc="Processing BioBERT"):
            if word in ['<pad>', '<unk>'] or word in glove_words:
                continue
                
            # If word not found in GloVe
            try:
    ...
    print(f"GloVe coverage: {glove_found_words} words ({glove_found_words/total_words*100:.2f}%)")
    ...
    print(f"Combined coverage: {(glove_found_words + biobert_found)/total_words*100:.2f}%")
    print("========================================\n")
    
    return embedding_matrix, {'glove': glove_found_words, 'biobert': biobert_found, 'random': random_initialized}
...
def train_eval_predict(args):
    ...
    try:
        ...
        if args.use_glove:
            print(f"- GloVe: {embedding_stats['glove']} words")
        if args.use_biobert:
        ...
        if args.train or not os.path.exists(args.model_path):
            ...
            best_model = train_model(
                ...
                train_dataloader, 
                val_dataloader, 
                model_metadata,  # Pass the metadata instead of just word_to_idx
            ...
            if 'embedding_stats' in checkpoint:
                ...
                if checkpoint.get('use_glove', False):
                    print(f"- GloVe: {checkpoint['embedding_stats']['glove']} words")
                if checkpoint.get('use_biobert', False):
                    print(f"- BioBERT: {checkpoint['embedding_stats']['biobert']} words")
                print(f"- Random: {checkpoint['embedding_stats']['random']} words")
